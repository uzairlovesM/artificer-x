...
class CitationLedger {
    ...
    fun add(citation:Citation){require(citation.id.isNotBlank());entries[citation.id]=citation}
    fun get(id:String)=entries[id]
    fun all():List<Citation>=entries.values.toList()
    fun unresolvedClaims(claims:List<String>):List<String>=claims.filter{claim->entries.values.none{it.claim==claim}}
}
