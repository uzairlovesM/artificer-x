...
data class CanvasRepairPlannerInput(
    val id: String,
...
data class CanvasRepairPlannerResult(
    val id: String,
    val score: Double,
...
class CanvasRepairPlanner(
    private val sensitivity: Double = 1.0,
    private val floor: Double = 0.000001,
) {
    private val history = ArrayDeque<CanvasRepairPlannerResult>()
    private val counters = LinkedHashMap<String, Long>()
    private val cache = LinkedHashMap<String, CanvasRepairPlannerResult>()

    fun evaluate(input: CanvasRepairPlannerInput): CanvasRepairPlannerResult {
        ...
        val result = CanvasRepairPlannerResult(
    ...
    fun rank(inputs: List<CanvasRepairPlannerInput>): List<CanvasRepairPlannerResult> =
        inputs.map(::evaluate).sortedByDescending { it.score }

    fun select(inputs: List<CanvasRepairPlannerInput>, limit: Int): List<CanvasRepairPlannerResult> =
        rank(inputs).filter { it.selected }.take(limit.coerceAtLeast(0))

    fun aggregate(results: List<CanvasRepairPlannerResult>): CanvasRepairPlannerResult {
        if (results.isEmpty()) return CanvasRepairPlannerResult("aggregate", 0.0, emptyList(), false, listOf("empty"), emptyMap())
        ...
        return CanvasRepairPlannerResult(
    ...
    private fun key(input: CanvasRepairPlannerInput): String = input.id + ":" + input.values.hashCode() + ":" + input.weight + ":" + input.priority
...
class CanvasRepairPlannerStage1(private val bias: Double = 0.05) {
...
class CanvasRepairPlannerStage2(private val bias: Double = 0.1) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage3(private val bias: Double = 0.15) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage4(private val bias: Double = 0.2) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage5(private val bias: Double = 0.25) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage6(private val bias: Double = 0.3) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage7(private val bias: Double = 0.35) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage8(private val bias: Double = 0.4) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage9(private val bias: Double = 0.45) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage10(private val bias: Double = 0.5) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage11(private val bias: Double = 0.55) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage12(private val bias: Double = 0.6) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage13(private val bias: Double = 0.65) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasRepairPlannerStage14(private val bias: Double = 0.7) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
