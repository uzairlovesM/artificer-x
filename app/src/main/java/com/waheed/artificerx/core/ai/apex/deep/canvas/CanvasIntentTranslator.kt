...
data class CanvasIntentTranslatorInput(
    val id: String,
...
data class CanvasIntentTranslatorResult(
    val id: String,
    val score: Double,
...
class CanvasIntentTranslator(
    private val sensitivity: Double = 1.0,
    private val floor: Double = 0.000001,
) {
    private val history = ArrayDeque<CanvasIntentTranslatorResult>()
    private val counters = LinkedHashMap<String, Long>()
    private val cache = LinkedHashMap<String, CanvasIntentTranslatorResult>()

    fun evaluate(input: CanvasIntentTranslatorInput): CanvasIntentTranslatorResult {
        ...
        val result = CanvasIntentTranslatorResult(
    ...
    fun rank(inputs: List<CanvasIntentTranslatorInput>): List<CanvasIntentTranslatorResult> =
        inputs.map(::evaluate).sortedByDescending { it.score }

    fun select(inputs: List<CanvasIntentTranslatorInput>, limit: Int): List<CanvasIntentTranslatorResult> =
        rank(inputs).filter { it.selected }.take(limit.coerceAtLeast(0))

    fun aggregate(results: List<CanvasIntentTranslatorResult>): CanvasIntentTranslatorResult {
        if (results.isEmpty()) return CanvasIntentTranslatorResult("aggregate", 0.0, emptyList(), false, listOf("empty"), emptyMap())
        ...
        return CanvasIntentTranslatorResult(
    ...
    private fun key(input: CanvasIntentTranslatorInput): String = input.id + ":" + input.values.hashCode() + ":" + input.weight + ":" + input.priority
...
class CanvasIntentTranslatorStage1(private val bias: Double = 0.05) {
...
class CanvasIntentTranslatorStage2(private val bias: Double = 0.1) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage3(private val bias: Double = 0.15) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage4(private val bias: Double = 0.2) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage5(private val bias: Double = 0.25) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage6(private val bias: Double = 0.3) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage7(private val bias: Double = 0.35) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage8(private val bias: Double = 0.4) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage9(private val bias: Double = 0.45) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage10(private val bias: Double = 0.5) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage11(private val bias: Double = 0.55) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage12(private val bias: Double = 0.6) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage13(private val bias: Double = 0.65) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasIntentTranslatorStage14(private val bias: Double = 0.7) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
