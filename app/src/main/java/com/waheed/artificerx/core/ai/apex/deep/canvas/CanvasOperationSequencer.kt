...
data class CanvasOperationSequencerInput(
    val id: String,
...
data class CanvasOperationSequencerResult(
    val id: String,
    val score: Double,
...
class CanvasOperationSequencer(
    private val sensitivity: Double = 1.0,
    private val floor: Double = 0.000001,
) {
    private val history = ArrayDeque<CanvasOperationSequencerResult>()
    private val counters = LinkedHashMap<String, Long>()
    private val cache = LinkedHashMap<String, CanvasOperationSequencerResult>()

    fun evaluate(input: CanvasOperationSequencerInput): CanvasOperationSequencerResult {
        ...
        val result = CanvasOperationSequencerResult(
    ...
    fun rank(inputs: List<CanvasOperationSequencerInput>): List<CanvasOperationSequencerResult> =
        inputs.map(::evaluate).sortedByDescending { it.score }

    fun select(inputs: List<CanvasOperationSequencerInput>, limit: Int): List<CanvasOperationSequencerResult> =
        rank(inputs).filter { it.selected }.take(limit.coerceAtLeast(0))

    fun aggregate(results: List<CanvasOperationSequencerResult>): CanvasOperationSequencerResult {
        if (results.isEmpty()) return CanvasOperationSequencerResult("aggregate", 0.0, emptyList(), false, listOf("empty"), emptyMap())
        ...
        return CanvasOperationSequencerResult(
    ...
    private fun key(input: CanvasOperationSequencerInput): String = input.id + ":" + input.values.hashCode() + ":" + input.weight + ":" + input.priority
...
class CanvasOperationSequencerStage1(private val bias: Double = 0.05) {
...
class CanvasOperationSequencerStage2(private val bias: Double = 0.1) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage3(private val bias: Double = 0.15) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage4(private val bias: Double = 0.2) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage5(private val bias: Double = 0.25) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage6(private val bias: Double = 0.3) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage7(private val bias: Double = 0.35) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage8(private val bias: Double = 0.4) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage9(private val bias: Double = 0.45) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage10(private val bias: Double = 0.5) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage11(private val bias: Double = 0.55) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage12(private val bias: Double = 0.6) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage13(private val bias: Double = 0.65) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class CanvasOperationSequencerStage14(private val bias: Double = 0.7) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
