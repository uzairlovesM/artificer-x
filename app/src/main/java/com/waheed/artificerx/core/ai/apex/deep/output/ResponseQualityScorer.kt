...
data class ResponseQualityScorerInput(
    val id: String,
...
data class ResponseQualityScorerResult(
    val id: String,
    val score: Double,
...
class ResponseQualityScorer(
    ...
    private val history = ArrayDeque<ResponseQualityScorerResult>()
    private val counters = LinkedHashMap<String, Long>()
    private val cache = LinkedHashMap<String, ResponseQualityScorerResult>()

    fun evaluate(input: ResponseQualityScorerInput): ResponseQualityScorerResult {
        ...
        val result = ResponseQualityScorerResult(
    ...
    fun rank(inputs: List<ResponseQualityScorerInput>): List<ResponseQualityScorerResult> =
        inputs.map(::evaluate).sortedByDescending { it.score }

    fun select(inputs: List<ResponseQualityScorerInput>, limit: Int): List<ResponseQualityScorerResult> =
        rank(inputs).filter { it.selected }.take(limit.coerceAtLeast(0))

    fun aggregate(results: List<ResponseQualityScorerResult>): ResponseQualityScorerResult {
        if (results.isEmpty()) return ResponseQualityScorerResult("aggregate", 0.0, emptyList(), false, listOf("empty"), emptyMap())
        ...
        return ResponseQualityScorerResult(
    ...
    private fun key(input: ResponseQualityScorerInput): String = input.id + ":" + input.values.hashCode() + ":" + input.weight + ":" + input.priority
...
class ResponseQualityScorerStage1(private val bias: Double = 0.05) {
...
class ResponseQualityScorerStage2(private val bias: Double = 0.1) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage3(private val bias: Double = 0.15) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage4(private val bias: Double = 0.2) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage5(private val bias: Double = 0.25) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage6(private val bias: Double = 0.3) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage7(private val bias: Double = 0.35) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage8(private val bias: Double = 0.4) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage9(private val bias: Double = 0.45) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage10(private val bias: Double = 0.5) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage11(private val bias: Double = 0.55) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage12(private val bias: Double = 0.6) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage13(private val bias: Double = 0.65) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
...
class ResponseQualityScorerStage14(private val bias: Double = 0.7) {
    private val state = LinkedHashMap<String, Double>()
    fun transform(values: List<Double>, multiplier: Double = 1.0): List<Double> {
        val safe = values.map { if (it.isFinite()) it else 0.0 }
        val out = ArrayList<Double>(safe.size)
