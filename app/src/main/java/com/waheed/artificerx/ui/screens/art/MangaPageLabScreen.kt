...
fun MangaPageLabScreen(onBack:()->Unit){
 ...
 Scaffold(topBar = {WorkspaceTopBar("Manga Page Studio", "Panels, gutters, captions and reading flow", onBack)}){pad->Column(Modifier.fillMaxSize().padding(pad), verticalArrangement = Arrangement.spacedBy(8.dp)){
  Row(Modifier.padding(10.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)){Button(onClick = {panels.add(Rect(.15f, .15f, .5f, .4f))}){Text("Add panel")};Button(onClick = {if(panels.size>1)panels.removeAt(panels.lastIndex)}){Text("Remove")};Button(onClick = {mode = !mode}){Text(if(mode)"RTL" else "LTR")}}
